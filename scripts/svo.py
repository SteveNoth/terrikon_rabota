...
Два разных вопроса, которые сюда не входят:
    ...
    trust_score — «можно ли верить вакансии?»  Вызывать его нельзя.
                  Скрытый набор — не мошенничество в смысле завышенной
                  зп сварщика: это другой тип объявления.
...
Порядок (как в 11.1; process_post вызывает этот слой сам):
...
def apply_svo_gate(
    ...
    """СВО-ворота для тестов и для process_post.

    process_post в части 2 этапа 14 вызывает тот же порядок:
