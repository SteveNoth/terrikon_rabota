...
- Последний завершённый этап: 18A (код и сухой прогон; коммит пользователь не просил)
...
- Этап 18A: миграцию `20260830230000_trudvsem` нужно накатить на Supabase (`npx prisma migrate deploy`), иначе прод не примет `TRUDVSEM`. `CRON_SECRET` / `SITE_URL` в Vercel и GitHub Secrets по-прежнему ☐ — без них upload и archive-missing на проде не откроются. С этой машины страница API `limit≥10` часто таймаутится; парсер уменьшает шаг до 5, прокси не используем.
