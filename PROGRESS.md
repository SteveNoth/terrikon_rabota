...
## Где я сейчас
- Текущий этап: 18 (парсер сайтов предприятий) — **критерии «готово» закрыты** на ветке `stage-18-parser-web`
- Последний завершённый этап: 18 (код, сухой прогон, коммит на этой ветке)
- Последний коммит: Этап 18 на `stage-18-parser-web` (от `stage-17-parser-tg`)
- Ветка этапа: `stage-18-parser-web`
- Порядок дальше: **этап 18A** (открытые данные «Работы России» / ЦЗН, не HTML Мой ЦЗН) — код не начинать, пока нет «готово». Потом **этап 19** (админка).
...
- Этап 18: если появятся свои страницы вакансий Стирола, Горловского машзавода и других заводов (HTML, не hh.ru и не Avito) — дописать `config_web.json`. ЮГМК ведёт на hh.ru. Avito / OK — не HTML. Мой ЦЗН — Этап 18A через `opendata.trudvsem.ru`, не селекторы.
