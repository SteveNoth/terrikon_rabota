...
## Где я сейчас
- Текущий этап: 17 (парсер Telegram) — два канала в `sources_tg.json`, локальный `--dry-run` нашёл вакансии (IPv6: провайдер не пускает IPv4 до датацентров Telegram). Жду секреты в GitHub Actions и «готово»
- Последний завершённый этап: 16 (парсер ВК, ветка `stage-16-parser-vk`) — код есть, на сайте и в Actions ещё нужны секреты
- Последний коммит: Этап 15 (ветка `stage-15-parser-upload`); этапы 16–17 пока в рабочих ветках
- Ветка этапа: `stage-17-parser-tg` (от `stage-16-parser-vk`)
- Порядок дальше: **этап 18** (парсер сайтов предприятий) — не начинать, пока нет «готово».
...
- Этап 16: парсер ВК. Только `wall.get` (официальный API, `access_token` + `v=5.199`). Список групп `scripts/sources_vk.json` (8 горловских групп, `default_city=gorlovka`). `scripts/parser_vk.py` отдаёт текст и URL картинок в `process_post`, карточку сам не собирает. `--dry-run` / `--limit`. Workflow `.github/workflows/parser-vk.yml` (cron `0 */3 * * *`, Tesseract `rus`, секреты `VK_TOKEN` / `CRON_SECRET` / `SITE_URL`, артефакт `logs/`). Выключатель `SOURCE_VK_ENABLED`. Решение: `docs/SOURCES-LEGAL.md`. Токен в git не кладём.
...
- Python: 3.14.6; виртуальное окружение `.venv`; зависимости `requirements.txt` (`requests`, `pytest`, `pytesseract`, `pillow`, `psycopg`, `telethon`). Tesseract — системная программа, не pip
...
- ВКонтакте — только официальный `wall.get`; Telegram — Telethon + StringSession в Secrets, не файл `*.session` и не HTML t.me (см. docs/DECISIONS.md, docs/SOURCES-LEGAL.md)
...
- Этап 16: в `scripts/sources_vk.json` 8 включённых горловских групп. Нужен `CRON_SECRET` в Vercel + новый деплой. Код этапа ещё не запушен — без этого кнопки `parser-vk` в Actions нет.
